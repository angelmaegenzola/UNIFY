/* ============================================================
   UNIFY — student_messages.js  (REWRITTEN v2)
   Matches officer_messages layout exactly.
...
const SM = (() => {
  ...
  const BASE    = `index.php?page=student_messages&club_id=${CFG.clubId}`;
  ...
  function appendMessages(msgs) {
    ...
    msgs.forEach(msg => {
      const showAvatar = msg.sender_id !== prevUserId;
      const el = buildMsgEl(msg, showAvatar);
  ...
  function buildMsgEl(msg, showAvatar) {
    ...
    // Avatar HTML - use profile picture if available
    let avatarHtml;
    if (msg.avatar) {
    ...
    if (isOwn) {
      // Own messages — bubble on right
      wrap.innerHTML = `
        <div class="msg-body">
          <div class="msg-bubble-wrap">
            ${canDel ? `<button class="msg-del-btn" title="Delete" onclick="SM.confirmDelete(${msg.id}, ${isDM})"><i class="fas fa-trash-can"></i></button>` : ''}
